namespace MpMigrate.Data.Dal
{
    ...
    public class Plesk_10X_MySql : DboFactory
    {
        ...
        public override List<Domain> GetDomains()
        {
            ...
            using (MySqlConnection _conn = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand _cmd = new MySqlCommand(@"SELECT 
                                                             ...
                                                             accounts.password, 
                                                             clients.login, 
                                                             accounts.password as DomainPass, 
        ...
        public override List<DomainAlias> GetDomainAliases(string domainName)
        {
            ...
            {
                _conn.Open();
                using (MySqlCommand _cmd = new MySqlCommand(@"SELECT domainaliases.name as alias, domains.name AS domain, domainaliases.status
                                                                FROM  domainaliases 
                                                            INNER JOIN domains ON domainaliases.dom_id = domains.id
                                                            WHERE domainaliases.status = 0 AND domains.name = @NAME", _conn))
                {
                    _cmd.Parameters.AddWithValue("@NAME", domainName);
                    ...
                    using (MySqlDataReader _read = _cmd.ExecuteReader())
                    {
                        while (_read.Read())
                        {
                            ...
                            _d.Status = Convert.ToInt64(DataExtensions.GetColumnValue<UInt32>(_read, "status"));
        ...
        public override List<Reseller> GetResellers()
        {
            ...
            {
                _conn.Open();
                using (MySqlCommand _cmd = new MySqlCommand(@"SELECT  C.id, cname, pname, login, accounts.password, phone, fax, email, address, city, state, pcode, country
                               FROM clients C LEFT JOIN accounts ON accounts.id = C.account_id WHERE C.`type`='reseller' OR C.`type`='client'", _conn))
                {
                    using (MySqlDataReader _read = _cmd.ExecuteReader())
                    {
                        while (_read.Read())
