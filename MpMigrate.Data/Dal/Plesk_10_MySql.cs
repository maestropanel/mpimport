namespace MpMigrate.Data.Dal
{
    ...
    public class Plesk_10_MySql : DboFactory
    {
                ...
                using (MySqlCommand _cmd = new MySqlCommand(@"SELECT login, passwd FROM  db_users WHERE   db_id = @ID AND (status = 'normal')", _conn))
                {
                    ...
                    using (MySqlDataReader _read = _cmd.ExecuteReader())
                    {
                        while (_read.Read())
                        {
                            ...
                            _d.Password = securePassword ? DataHelper.GetPassword() : DataExtensions.GetColumnValue<string>(_read, "passwd");
        ...
        public override List<Reseller> GetResellers()
        {
            ...
            {
                _conn.Open();
                using (MySqlCommand _cmd = new MySqlCommand(@"SELECT  C.id, cname, pname, login, passwd, phone, fax, email, address, city, state, pcode, country
                               FROM clients C WHERE C.`type`='reseller' OR C.`type`='client'", _conn))
                {
                    using (MySqlDataReader _read = _cmd.ExecuteReader())
                    {
                        while (_read.Read())
