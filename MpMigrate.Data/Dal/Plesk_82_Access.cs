namespace MpMigrate.Data.Dal
{
    ...
    public class Plesk_82_Access : DboFactory
    {
        ...
        public override DnsZone GetDnsZone(string domainName)
        {
            ...
            {
                _conn.Open();
                using (OleDbCommand _cmd = new OleDbCommand(@"SELECT dns_zone.id, dns_zone.name, dns_zone.displayName, dns_zone.email, 
                                        dns_zone.status, dns_zone.type, dns_zone.ttl, dns_zone.ttl_unit, dns_zone.refresh, dns_zone.refresh_unit, dns_zone.retry, 
                                        dns_zone.retry_unit, dns_zone.expire, dns_zone.expire_unit, dns_zone.minimum, dns_zone.minimum_unit
        ...
        public override List<DatabaseUser> GetDatabaseUsers(int database_id)
        {
            ...
            {
                _conn.Open();
                using (OleDbCommand _cmd = new OleDbCommand(@"SELECT login, passwd FROM db_users WHERE (db_id = ?)", _conn))
                {
        ...
        public override List<Reseller> GetResellers()
        {
            ...
            {
                _conn.Open();
                using (OleDbCommand _cmd = new OleDbCommand(@"SELECT id, cr_date, cname, pname, login, passwd, status, phone, fax, email, 
                                                            address, city, state, pcode, country, locale, limits_id, params_id, perm_id, 
                                                            pool_id, logo_id, tmpl_id, sapp_pool_id, uid, ownership FROM clients", _conn))
                {
                    using (OleDbDataReader _read = _cmd.ExecuteReader())
                    {
                        while (_read.Read())
        ...
        public override HostLimit ResellerLimits(string clientName)
        {
            ...
            using (OleDbConnection _conn = new OleDbConnection(connectionString))
            {
                _conn.Open();
                using (OleDbCommand _cmd = new OleDbCommand(@"SELECT L.limit_name, L.[value] 
                                            FROM (clients C LEFT OUTER JOIN limits L ON L.id = C.limits_id) 
                                WHERE (C.login = ?) AND (L.limit_name <> NULL)", _conn))
                {
