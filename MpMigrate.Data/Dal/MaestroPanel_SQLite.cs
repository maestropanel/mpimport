namespace MpMigrate.Data.Dal
{
    ...
    public class MaestroPanel_SQLite : DboFactory
    {
        ...
        private bool isForwarding(int domainId)
        {
            ...
            using (SQLiteConnection _conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT COUNT(*) FROM DomainRedirection WHERE DomainId = @ID AND Enabled = 1", _conn))
                {
                    _cmd.Parameters.AddWithValue("@ID", domainId);
                    forwarding = ((long)_cmd.ExecuteScalar() > 0);                    
                }
        ...
        public override List<Email> GetEmails(string domainName)
        {
            ...
            using (SQLiteConnection _conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT DomainPostofficeUser.Username, DomainPostofficeUser.Password, 
                                                                    DomainPostofficeUser.AutoResponderSubject, 
                                                                        DomainPostofficeUser.AutoResponderMessage, DomainPostofficeUser.Quota, 
                                                                          DomainPostofficeUser.Status, DomainPostofficeUser.RedirectionStatus, 
                                                                ...
                                                                WHERE (Domain.Name = @NAME)", _conn))
                {
                    ...
                    using (SQLiteDataReader _read = _cmd.ExecuteReader())
                    {
                        while (_read.Read())
                        {
                            ...
                            da.Quota = DataExtensions.GetColumnValue<Int64>(_read, "Quota");
        ...
        public override List<DnsZoneRecord> GetZoneRecords(string domainName)
        {
            ...
            using (SQLiteConnection _conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT Domain.Name AS DomainName, DomainMsDns.RecordType, DomainMsDns.Name, 
        ...
        public override List<Database> GetDatabases(string domainName)
        {
            ...
            using (SQLiteConnection _conn = new SQLiteConnection(connectionString))
            {
                ...
                #region MSSQL 2008
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT DomainMsSQL.Id, DomainMsSQL.Name, Domain.Name AS DomainName FROM Domain INNER JOIN DomainMsSQL ON Domain.DomainId = DomainMsSQL.DomainId  
                                                            WHERE (Domain.Name = @NAME)", _conn))
                {
                    _cmd.Parameters.AddWithValue("@NAME", domainName);
                    ...
                    using (SQLiteDataReader _read = _cmd.ExecuteReader())
                    {
                        ...
                        {
                            var da = new Database();
                            da.Id = Convert.ToInt32(DataExtensions.GetColumnValue<Int64>(_read, "Id"));               
                            da.Name = DataExtensions.GetColumnValue<String>(_read, "Name");
                            da.Domain = domainName;
                            da.DbType = "mssql";
                ...
                #region MSSQL 2012
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT DomainMsSQL2012.Id, DomainMsSQL2012.Name, DomainMsSQL2012.Id, Domain.Name AS DomainName FROM Domain 
                ...
                #region MYSQL
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT DomainMySQL.Id, DomainMySQL.Name, DomainMySQL.Id, Domain.Name AS DomainName 
        ...
        public override List<DatabaseUser> GetDatabaseUsers(int database_id)
        {
            ...
            using (SQLiteConnection _conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT Password, Username FROM DomainMsSQLUser WHERE (DatabaseId = @ID)", _conn))
                {
        ...
        public List<DatabaseUser> GetDatabaseUsersMsSQL2012(int database_id)
        {
            ...
            using (SQLiteConnection _conn = new SQLiteConnection(connectionString))
            {
                ...
                using (SQLiteCommand _cmd = new SQLiteCommand(@"SELECT Username, Password, DatabaseId FROM DomainMsSQL2012User WHERE (DatabaseId = @ID)", _conn))
                {
                    _cmd.Parameters.AddWithValue("@ID", database_id);
