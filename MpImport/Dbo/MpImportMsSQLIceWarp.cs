        ...
        public override List<Email> GetEmails(string domainName)
        {
            ...
            using (SqlConnection _conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand _cmd = new SqlCommand(@"SELECT Domain.Name AS name, Pu.Username AS mail_name, Pu.Password AS password, 
                                                                    Pu.DiskQuota AS mbox_quota
                                                            FROM Domain LEFT OUTER JOIN
                                                                DomainIceWarp AS Po ON Po.DomainId = Domain.DomainId RIGHT OUTER JOIN
                                                                DomainIceWarpUser AS Pu ON Pu.IceWarpDomainId = Po.Id
